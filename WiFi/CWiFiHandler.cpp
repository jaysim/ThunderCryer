...
namespace mbed_cc3000 {
  ...
  msg_t CWiFiHandler::main(void) {
    ...
    while(1){
      ...
      SDU2.print("cc3000 NTP client. \r\n");
      print_cc3000_info();

      SDU2.print("Attempting SSID Connection. \r\n");

      if (user_info.FTC == 1) {
        wifi._wlan.ioctl_set_connection_policy(0, 1, 1);
      } else {
        SDU2.print("Smart config is not set, starting configuration. \r\n");
        do_FTC();
        SDU2.print("Smart config is set. Please restart your board. \r\n");
        while(1);
      }

      SDU2.print("DHCP request \r\n");
      while (wifi.is_dhcp_configured() == false) {
        sleep(MS2ST(500));
        SDU2.print("  Waiting for dhcp to be set. \r\n");
      }


      while(wifi.is_connected()){

        SDU2.print("Getting time, 10s timeout. \r\n");
        if (ntp.setTime("0.uk.pool.ntp.org") == 0)
        {
          SDU2.print("Set time successfully. \r\n");
          time_t ctTime;
          ctTime = time(NULL);
          SDU2.print("Time is set to (UTC): %s \r\n", ctime(&ctTime));
        }
        else
        {
          SDU2.print("Error. \r\n");
  ...
  void CWiFiHandler::do_FTC(void) {
    SDU2.print("Running First Time Configuration \r\n");
  ...
  void CWiFiHandler::connect_to_ssid(char *ssid, char *key, unsigned char sec_mode) {
    SDU2.print("Connecting to SSID: %s. Timeout is 10s. \r\n",ssid);
    if (wifi.connect_to_AP((uint8_t *)ssid, (uint8_t *)key, sec_mode) == true) {
      SDU2.print(" Connected. \r\n");
    } else {
      SDU2.print(" Connection timed-out (error). Please restart. \r\n");
      while(1);
    }
  }
  ...
  void CWiFiHandler::print_cc3000_info() {
    ...
    SDU2.print("MAC address + cc3000 info \r\n");
    wifi.get_user_file_info((uint8_t *)&user_info, sizeof(user_info));
    wifi.get_mac_address(myMAC);
    SDU2.print(" MAC address %02x:%02x:%02x:%02x:%02x:%02x \r\n \r\n", myMAC[0], myMAC[1], myMAC[2], myMAC[3], myMAC[4], myMAC[5]);

    SDU2.print(" FTC        %i \r\n",user_info.FTC);
    SDU2.print(" PP_version %i.%i \r\n",user_info.PP_version[0], user_info.PP_version[1]);
    SDU2.print(" SERV_PACK  %i.%i \r\n",user_info.SERV_PACK[0], user_info.SERV_PACK[1]);
    SDU2.print(" DRV_VER    %i.%i.%i \r\n",user_info.DRV_VER[0], user_info.DRV_VER[1], user_info.DRV_VER[2]);
    SDU2.print(" FW_VER     %i.%i.%i \r\n",user_info.FW_VER[0], user_info.FW_VER[1], user_info.FW_VER[2]);
  }
